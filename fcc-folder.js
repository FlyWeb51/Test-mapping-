...
module.exports = async (req, res) => {
  try {
    ...
    const data = await cached(`ie:${cand}:${cycle}`, H6, async () => {
      const r = await (await fetch(`https://api.open.fec.gov/v1/schedules/schedule_e/by_candidate/?api_key=${FEC_KEY()}&candidate_id=${cand}&cycle=${cycle}&election_full=true&per_page=10`)).json();
